...
# ThirdModelVIT8Training.py
from ThirdModelVIT8Training import (
    SpineDataset,
    SpineDataloader,
    FeatureExtractor,
    Refiner,
...
def segmentation_metrics_per_sample_from_logits(
    ...
    logits:
        Shape (B, 1, D, H, W), raw model outputs before sigmoid.

    targets:
        Shape (B, 1, D, H, W), binary ground-truth voxel masks.
    """
...
def main():
    ...
    # -------------------------------------------------------------------------
    # Model: ViT-S/8 DAX feature extractor + refiner
    # -------------------------------------------------------------------------
    feature_extractor = FeatureExtractor(
        checkpoint_file=DAX_checkpoint,
    ...
    print(f"Loading trained reconstruction model from: {model_checkpoint}")
