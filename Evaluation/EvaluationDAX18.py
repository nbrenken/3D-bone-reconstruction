...
#
# Example:
# if your training file is ThirdModelTraining.py:
from ThirdModelDAX18Training import (
    SpineDataset,
    SpineDataloader,
    FeatureExtractor,
    Refiner,
...
def main():
    ...
    dax_checkpoint = Path(
        r"C:\Users\karlo\OneDrive\Desktop\DinoVert\dax-checkpoint-resnet18-version-a.pth"
    )

    model_checkpoint = Path(
        r"C:\Users\karlo\OneDrive\Desktop\DinoVert\ShirleySTUFF\models_dax18\best_model.pth"
    )

    output_csv = Path(
        r"C:\Users\karlo\OneDrive\Desktop\DinoVert\ShirleySTUFF\EVALDAX18\test_segmentation_metrics_results.csv"
    )
    ...
    if not model_checkpoint.exists():
        raise FileNotError(f"Model checkpoint not found: {model_checkpoint}")
    ...
    model = ReconstructionModel(dax, refiner).to(device)
