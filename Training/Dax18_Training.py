...
class FeatureExtractor(torch.nn.Module):
    def __init__(self, checkpoint_file: Path, device=None):
        super().__init__()
        ...
        self.DAX = models.resnet18(weights=None)

        # Your DAX checkpoint has no classification head, so remove it before loading.
        self.DAX.fc = nn.Identity()

        # Also remove possible fc entries just in case another checkpoint contains them.
        teacher_dict = {
            k: v for k, v in teacher_dict.items()
            if not k.startswith("fc.")
        }

        self.DAX.load_state_dict(teacher_dict, strict=True)

        # Remove unused pooling/head modules after loading.
        ...
        def forward_no_flatten(x):
            ...
            x1 = self.DAX.layer1(x)  # (B, 64, 64, 64)
            x2 = self.DAX.layer2(x1) # (B, 128, 32, 32)
            x3 = self.DAX.layer3(x2) # (B, 256, 16, 16)
            x4 = self.DAX.layer4(x3) # (B, 512, 8, 8)
    ...
    def forward(self, x, P_matrices, grid_size=80):
        ...
        return torch.cat([grid1, grid2, grid3, grid4], dim=1)  # (B, 64, D, D, D)
...
def main():
    ...
    DAX_checkpoint = Path(r"C:\Users\karlo\OneDrive\Desktop\DinoVert\dax-checkpoint-resnet18-version-a.pth")
    #DAX_checkpoint = Path(r"C:\Users\karlo\OneDrive\Desktop\DinoVert\dax-checkpoint-resnet50-version-b.pth")
    ...
    dax = FeatureExtractor(DAX_checkpoint, device=device).to(device)
