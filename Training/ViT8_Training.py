...
class FeatureExtractor(torch.nn.Module):
    def __init__(
        ...
        self.DAX_model_type = DAX_model_type.lower()
        self.is_vit = self.DAX_model_type.startswith("vit")
        ...
        if self.is_vit:
            # Accept names like:
            # "vit_s_8", "vit-s-8", "vit_small_8", "ViT-S-8"
            name = self.DAX_model_type.replace("_", "-")

            if name in ["vit-s-8", "vit-small-8"]:
                self.patch_size = 8
                self.DAX = vits.vit_small(patch_size=8)
            elif name in ["vit-s-16", "vit-small-16"]:
                self.patch_size = 16
                self.DAX = vits.vit_small(patch_size=16)
            elif name in ["vit-t-8", "vit-tiny-8"]:
                self.patch_size = 8
                self.DAX = vits.vit_tiny(patch_size=8)
            elif name in ["vit-t-16", "vit-tiny-16"]:
                self.patch_size = 16
                self.DAX = vits.vit_tiny(patch_size=16)
            else:
                raise ValueError(f"Unknown ViT model type: {DAX_model_type}")
            ...
            embed_dim = self.DAX.embed_dim  # 192 for ViT-T, 384 for ViT-S

            # Four transformer layers, each reduced to 16 channels.
            self.reducer1 = nn.Conv2d(embed_dim, 16, kernel_size=1)
            self.reducer2 = nn.Conv2d(embed_dim, 16, kernel_size=1)
            self.reducer3 = nn.Conv2d(embed_dim, 16, kernel_size=1)
            self.reducer4 = nn.Conv2d(embed_dim, 16, kernel_size=1)

        else:
            # Original ResNet path
            teacher_dict = {
            ...
            self.DAX = models.__dict__[self.DAX_model_type](weights=None)
    ...
    def _vit_tokens_to_map(self, tokens, H, W):
        """
        tokens: (B*V, num_patches + 1, embed_dim)
        returns: (B*V, embed_dim, H_patch, W_patch)
        """
        ...
        if N != expected:
            raise RuntimeError(
                f"Unexpected number of ViT patch tokens: got {N}, expected {expected}. "
                f"Input size was {H}x{W}, patch size {self.patch_size}."
            )

        return patch_tokens.transpose(1, 2).reshape(Bv, C, H_patch, W_patch)
    ...
    def forward(self, x, P_matrices, grid_size=80):
        ...
        with torch.no_grad():
            if self.is_vit:
                # Last 4 transformer blocks, each shaped as token sequence.
                token_layers = self.DAX.get_intermediate_layers(x_flat, n=4)

                x1 = self._vit_tokens_to_map(token_layers[0], H, W)
                x2 = self._vit_tokens_to_map(token_layers[1], H, W)
                x3 = self._vit_tokens_to_map(token_layers[2], H, W)
                x4 = self._vit_tokens_to_map(token_layers[3], H, W)
            else:
...
def debug_drr_dax(dataset, dax, num_samples=3):
    ...
    for sample_idx in range(min(num_samples, len(dataset))):
        ...
        with torch.no_grad():
            if dax.is_vit:
                token_layers = dax.DAX.get_intermediate_layers(x, n=4)
                _, _, H, W = x.shape
                x1 = dax._vit_tokens_to_map(token_layers[0], H, W)
                x2 = dax._vit_tokens_to_map(token_layers[1], H, W)
                x3 = dax._vit_tokens_to_map(token_layers[2], H, W)
                x4 = dax._vit_tokens_to_map(token_layers[3], H, W)
            else:
                x1, x2, x3, x4 = dax.DAX(x)
...
def main():
    ...
    if not DAX_checkpoint.exists():
        raise FileNotFoundError(f"DAX checkpoint not found: {DAX_checkpoint}")
